  ...
  datas: [
    {
      ...
      desc: "逐一将 [待排序列的元素] 依次与 [已排序列的元素] 进行比较，若小于则交换位置，直至不小于，则位于已排序列中的适当位置，适用于基本有序的序列",
    },
    {
      code: "shells",
      name: "希尔排序",
      en: "shells sort",
      desc: "又称缩小增量法，是插入排序的改进。先取一个小于n的增量d，将距离为d的值放入同一组，将序列 [分组] 并进行 [插入排序]，然后逐步缩小分组的规模，直至增量为1，最终完成排序",
    },
    {
      code: "selection",
      name: "选择排序",
    ...
    },
    {
      ...
      name: "堆排序",
      en: "heap sort",
      desc: "将待排序列构建成一个 [大顶堆]，然后将堆顶元素(最大值)与最后一个元素交换，再对剩余的元素循环执行以上步骤，每一轮比较都将 [最大元素] 放到最后，最大值归位",
    },
    {
      code: "bubble",
      name: "冒泡排序",
      en: "bubble sort",
      desc: "依次将 [相邻元素] 的大值交换至后位，再对剩余的元素循环执行以上步骤，每一轮比较都将 [最大元素] 放到最后，最大值归位",
    },
    {
      code: "quick",
      name: "快速排序",
      en: "quick sort",
      desc: "选定一个 [枢轴值] 将待排序列分成独立的左小右大的两部分，枢轴归位，然后再对这两部分递归地进行快速排序",
    },
    {
      code: "merge",
      name: "归并排序",
      en: "merging sort",
      desc: "将序列递归平均 [分组] ，然后 [合并] 两个已排序的子序列，即依次对比两个序列的值，将较小值计入次序并移动其指针，最终完成排序",
    },
  ],
});
const datas: any = ref([]);
...
// 生成数组
const generateBars = () => {
  datas.value = Array.from({ length: pageInfos.size }, (_, i) => ({
    id: i,
...
const heapify = async (n: number, i: number) => {
  ...
  let left = 2 * i + 1;
  let right = 2 * i + 2;
  if (left < n && datas.value[left].value > datas.value[largest].value)
    largest = left;
  if (right < n && datas.value[right].value > datas.value[largest].value)
    largest = right;
  if (largest !== i) {
  ...
  <div class="box-sort">
    <div class="box-contents">
      <div
        ...
        ref="container"
        :style="`width:${
          datas.length * (pageInfos.barWidth + pageInfos.gap * 2)
        }px`"
      >
    ...
    <div class="box-table">
      <table>
        <tr v-for="row in pageInfos.datas">
          <td>
            <a-button type="primary" @click="sort(row.code)"
