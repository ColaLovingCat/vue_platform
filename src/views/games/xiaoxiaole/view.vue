...
class GameMap {
  ...
  genCellMap() {
    ...
    cells.forEach(cell => {
      gameContainer.value.append(cell.instance);
    });
    console.log(cells);
    return this;
  }
...
let gameMap: GameMap = new GameMap(6, 10, 50);
onMounted(() => {
  gameMap = new GameMap(6, 10, 50);
  gameMap.genMatrix().genRandom();
  gameMap.init().genCellMap();
  gameMap.genLoop();
  //
  gameContainer.value.onclick = () => {
...
<template>
  <div class="box-games">
    ...
    <div class="box-infos">
      <button id="btn" @click="shuffle">shuffle</button>
